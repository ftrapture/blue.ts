...
const arr = ["youtube", "youtube music", "soundcloud"];
...
class Util {
    ...
    checkParamsValidity(nodePackets: any[], defaultPackets: any) {
    ...
    checkObjectValidity(options: { guildId: string; voiceChannel: string; textChannel: string }) {
    ...
    durationInMs(time: string | number) {
        ...
        const onlyString = this.timeString((time as string).toUpperCase()).trim();
        let get = "s";
        if(onlyString !== "")
          get = units.find(ar => ar.includes(onlyString)) as string;
        let result;
