...
var FilterManager = /** @class */ (function () {
    ...
    FilterManager.prototype.setBassboost = function (val) {
        if (!this.player)
            return this;
        if (val < 0 || val > 6)
            throw new Error('Bassboost value must be between 0 and 6');
        this.bassboost = val;
        var sinBassboost = Math.sin(val * (Math.PI / 6));
        var cosBassboost = Math.cos(val * (Math.PI / 6));
        var tanBassboost = Math.tan(val * (Math.PI / 6));
        var smoothBassboost = (sinBassboost + cosBassboost + tanBassboost) / 3;
        var bands = Array(13).fill(0).map(function (n, i) { return ({
            band: i,
